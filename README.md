...
Layer II's services are identified by transaction output: service id = sha256(txid + 1 byte integer of n), which convert service from transaction scope to script scope. so we can write any vailid complex scripts to operate our services. 
